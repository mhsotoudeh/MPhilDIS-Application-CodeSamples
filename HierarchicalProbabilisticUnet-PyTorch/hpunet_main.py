...
parser.add_argument("--multigpu", action='store_true', help="Whether or Not to Use MultiGPU for Data Parallelism")

parser.add_argument("--train_file", help="<Required> Path to the Training Set", required=True)
parser.add_argument("--train_size", type=int, help="Training Set Size (If not provided, the whole datapoints in the traininig file will be used)")

parser.add_argument("--val_file", help="Path to the Validation Set")
parser.add_argument("--val_size", type=int, help="Validation Set Size (If not provided, the whole datapoints in the validation file will be used)")
...
parser.add_argument("--beta", type=float, default=1.0, help="Beta Parameter (If Using ELBO Loss)")
# parser.add_argument("--beta_scheduler", action='store_true', help="Whether to Use Scheduler for Beta or Not (If Using ELBO Loss)")
parser.add_argument("--beta_asc_steps", type=int, help="Number of Ascending Steps (If Not Provided, Beta Will be Constant) (If Using ELBO Loss with Beta Scheduler)")
parser.add_argument("--beta_cons_steps", type=int, default=1, help="Number of Constant Steps (If Using ELBO Loss with Beta Scheduler)")
parser.add_argument("--beta_saturation_step", type=int, help="The Step at Which Beta Becomes Permanently 1 (If Using ELBO Loss with Beta Scheduler)")

parser.add_argument("--kappa_px", action='store_true', help="Kappa Parameter Type (If true, Kappa should be provided per pixel)")
parser.add_argument("--kappa", type=float, default=1.0, help="Kappa Parameter (If Using GECO Loss)")
parser.add_argument("--decay", type=float, default=0.9, help="EMA Decay Rate/Smoothing Factor (If Using GECO Loss)")
parser.add_argument("--update_rate", type=float, default=0.01, help="Lagrange Multiplier Update Rate (If Using GECO Loss)")

parser.add_argument("--k", type=float, help="k Value If Using top-k Mask for Reconstruction Loss")
...
parser.add_argument("--std_type", help="If Provided, Will Use LogLikelihood As Reconstruction (predict: Predict Standard Deviation / sample: Calculate Standard Deviation From Samples)")
parser.add_argument("--std_sample_num", type=int, help="(If std_type is sample) Number of Samples to Calculate Standard Deviation")
...
# torch.set_default_tensor_type(torch.cuda.DoubleTensor if args.device == 'cuda' else torch.DoubleTensor)
...
writer = SummaryWriter('runs/'+stamp+'/tb')


# Start Monitoring Memory Usage
tracemalloc.start()
os.mkdir('runs/{}/memory'.format(args.stamp))
...
# Initialize Model
if args.std_type is not None and args.std_type.lower() == 'predict':
    model = HPUNet( in_ch=args.in_ch, out_ch=args.out_ch+1, chs=args.intermediate_ch,
...
if args.multigpu is True and torch.cuda.device_count() > 1:
    print( "MultiGPU: Using {} GPUs".format(torch.cuda.device_count()) )
...
if args.loss_type.lower() == 'elbo':
    ...
    criterion = ELBOLoss(beta=beta_scheduler, k=args.k, topk_deterministic=args.topk_deterministic).to(device)

elif args.loss_type.lower() == 'geco':
    kappa = args.kappa
    if args.kappa_px is True:
        kappa *= args.pixels
    criterion = GECOLoss(kappa=kappa, decay=args.decay, update_rate=args.update_rate, device=device, k=args.k, topk_deterministic=args.topk_deterministic).to(device)
...
# Save Args
argsdict = vars(args)
with open('runs/{}/args.json'.format(stamp), 'w') as f:
    json.dump(argsdict, f)
with open('runs/{}/args.txt'.format(stamp), 'w') as f:
...
torch.save(model, 'runs/{}/model.pth'.format(stamp))

torch.save(criterion, 'runs/{}/loss.pth'.format(stamp))

with open('runs/{}/history.json'.format(stamp), 'w') as f:
    json.dump(history, f)